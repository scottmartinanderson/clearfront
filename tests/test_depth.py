"""Sweep-depth invariants shared by the web console and the terminal agent."""

from clearfront import depth
...
def test_rounds_ceiling_per_level():
    assert depth.rounds("faster") == 4
    assert depth.rounds("balanced") == 8
    assert depth.rounds("deeper") == 12
    # Unknown level falls back to the deeper ceiling.
    assert depth.rounds("bogus") == 12
...
def test_rounds_respects_env_ceiling(monkeypatch):
    ...
    assert depth.rounds("faster") == 4
    assert depth.rounds("balanced") == 6  # min(8, 6)
    assert depth.rounds("deeper") == 6


def test_normalize():
    assert depth.normalize("faster") == "faster"
    assert depth.normalize("balanced") == "balanced"
    assert depth.normalize("deeper") == "deeper"
    assert depth.normalize("bogus") == "deeper"
    assert depth.normalize(None) == "deeper"
...
def test_levels_match_console_descriptions():
    ...
    by_v = {level["v"]: level for level in depth.LEVELS}
...
def test_deeper_prompt_is_byte_identical():
    ...
    assert system_prompt_for_depth("deeper") == SYSTEM_PROMPT
    assert system_prompt_for_depth("bogus") == SYSTEM_PROMPT
...
def test_lighter_levels_swap_enrich_and_prepend_mode_line():
    for level in ("faster", "balanced"):
        shaped = system_prompt_for_depth(level)
...
def test_web_and_agent_share_the_round_ceiling():
    ...
    assert web_server._depth_rounds is depth.rounds
