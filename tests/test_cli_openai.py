...
"""
Unit tests for the OpenAI-compatible CLI arguments (clearfront/cli.py) and
the resulting OISRepl / OpenAICompatibleAgent wiring (clearfront/repl.py).
...
class TestBuildParserOpenaiArgs:
    ...
    def test_provider_openai_accepted(self):
        from clearfront.cli import _build_parser
...
# ---------------------------------------------------------------------------
# OISRepl, provider="openai" routes to OpenAICompatibleAgent
# ---------------------------------------------------------------------------


class TestReplOpenaiProviderWiring:
    """OISRepl(provider='openai') must construct an OpenAICompatibleAgent."""
    ...
    def test_anthropic_provider_creates_anthropic_agent(self):
        from clearfront.agent import OpenAICompatibleAgent, OISAgent
        from clearfront.repl import OISRepl

        repl = OISRepl(provider="anthropic")
        assert isinstance(repl._agent, OISAgent)
        assert not isinstance(repl._agent, OpenAICompatibleAgent)

    def test_ollama_provider_does_not_create_openai_agent(self):
        from clearfront.agent import OllamaAgent, OpenAICompatibleAgent
        from clearfront.repl import OISRepl

        repl = OISRepl(provider="ollama")
